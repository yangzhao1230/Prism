...
def hic_pre_processing(cell_type, folder):
    ...
    for chrm in all_chr:
        ...
        gr1 = 0
        # gr2 = 1000000
        gr2 = next((chrom for chrom in chromosomes if chrom.name == chromosome1), None).length  
        gc1 = 0
        # gc2 = 1000000
        gc2 = next((chrom for chrom in chromosomes if chrom.name == chromosome2), None).length
