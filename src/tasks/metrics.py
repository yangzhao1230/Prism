...
def extract_rationale_loss(
        ...
        bio_mask_weight=0.0,
        kl_loss_weight=0.0,
        intervention_loss_weight=0.0,
        **kwargs,
):
    # Paper Eq. L1: prediction loss — smooth L1 between predicted and observed expression
    loss_dict = {'pred_loss': F.smooth_l1_loss(inputs, target)}

    # Paper Eq. L2: intervention backdoor loss — smooth L1 on intervened context predictions
    if intervention_logits is not None and intervention_loss_weight != 0:
        loss_dict['intervention_loss'] = intervention_loss_weight * F.smooth_l1_loss(intervention_logits, target)
    ...
    # Paper Eq. L3: uniform diversity regularization — encourages n_context weight vectors to be distinct
    uniform_loss_weight = kwargs.get('uniform_loss_weight', 0.0)
    if uniform_loss_weight != 0:
        uniform_loss = kwargs.get('uniform_loss', None)
        if uniform_loss:
            loss_dict['uniform_loss'] = uniform_loss_weight * uniform_loss
    return loss_dict

def learnable_uniform_loss(inputs, target, uniform_loss=None, uniform_loss_weight=0.0, **kwargs):
    loss_dict = {'pred_loss': F.smooth_l1_loss(inputs, target)}
    if uniform_loss is not None and uniform_loss_weight != 0:
        loss_dict['uniform_loss'] = uniform_loss_weight * uniform_loss
    return loss_dict
...
output_metric_fns = {
    ...
    "learnable_uniform_loss": learnable_uniform_loss,
}
